...
router.post('/register',
    ...
    async (req, res) => {
        ...
        const { username, email, password, mpAlias } = req.body;
        ...
        try {
            ...
            const result = await query(
                ...
                [username, email, hash, mpAlias || null]
            );
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                // Timing-safe: siempre hashear para no revelar si el usuario existe
...
router.post('/admin/login',
    ...
    async (req, res) => {
        ...
        // Comparar en tiempo constante (anti timing attack)
        const userMatch = username === adminUser;
        const passMatch = await bcrypt.compare(password, await bcrypt.hash(adminPass, 1))
            .catch(() => password === adminPass);

        // Simple string compare is ok here since adminPass is our own value
        if (!userMatch || password !== adminPass) {
